...
const Category = ({ category }) => {
  const [ subCategories, setSubCategories ] = useState(subCategoryHC);
  const [ selectedItems, setSelectedItems ] = useState([]);

  const handleSubCategorySelect = id => {
    setSelectedItems(subCategories.find(subCategory => subCategory.id === id).articulos.map(articulo => ({ ...articulo })))
  };

  return (
    <div className={ 'category__main-container'}>
      <SubCategoryList subCategories={ subCategories } onSubCategorySelect={ handleSubCategorySelect } />
